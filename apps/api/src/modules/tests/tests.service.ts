...
export class TestsService {
  ...
  async updateRunStatus(
    ...
    // Auto-generate report when a test run reaches a terminal state
    if (terminalStatuses.includes(status)) {
      try {
        ...
        this.logger.log(`Auto-enqueued report generation for completed run ${saved.id}`);
      } catch (err) {
        this.logger.error(`Failed to enqueue report for run ${saved.id}: ${(err as Error).message}`);
      }
    }
